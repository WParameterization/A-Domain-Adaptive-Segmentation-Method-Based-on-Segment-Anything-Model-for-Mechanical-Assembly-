...
class ImageEncoderViT(nn.Module):
    def __init__(self,img_size: int = 1024, patch_size: int = 16, in_chans: int = 3,embed_dim: int = 768,depth: int = 12,num_heads: int = 12, mlp_ratio: float = 4.0,
        out_chans: int = 256, qkv_bias: bool = True, norm_layer: Type[nn.Module] = nn.LayerNorm,act_layer: Type[nn.Module] = nn.GELU, use_abs_pos: bool = True,  use_rel_pos: bool = False,
        rel_pos_zero_init: bool = True, window_size: int = 0, global_attn_indexes: Tuple[int, ...] = (), ) -> None:
        """
        Args:
        ...
        self.patch_embed_resnet = PatchEmbed(kernel_size=(1, 1), stride=(1, 1),in_chans=self.resnet.width * 16,embed_dim=embed_dim )#加上resnet50的主要作用就是，不应embed了, embed_dim=embed_dim, )
        ...
        if use_abs_pos:
            ...
            self.pos_embed_resnet=nn.Parameter(torch.zeros(1, img_size // patch_size, img_size // patch_size, embed_dim))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x1, features = self.resnet(x)

        x1 = self.patch_embed_resnet(x1)

        if self.pos_embed is not None:
            x1 = x1 + self.pos_embed_resnet
        for blk in self.blocks:
            x1 = blk(x1)

        x1 = self.neck(x1.permute(0, 3, 1, 2))




        # x = self.patch_embed(x)  # pre embed: [1, 3, 1024, 1024], post embed: [1, 64, 64, 768]
        # if self.pos_embed is not None:#z
        #     x = x + self.pos_embed
        # #print(x.shape,"x.shape")#torch.Size([4, 16, 16, 768]) x.shape
        # for blk in self.blocks:
        #     x = blk(x)
        # #print(x.shape,"ppppppppppp")#torch.Size([4, 16, 16, 768]) ppppppppppp
